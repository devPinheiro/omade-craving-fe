import { PaymentLoader, type PaymentStatus } from '@/components/ui/PaymentLoader'
import { type CheckoutFormData, checkoutSchema, formatPhoneNumber } from '@/lib/checkout-validation'
import { calculateDeliveryFee, formatAmountForDisplay, paystackService } from '@/services/paystack'
import { useCartStore } from '@/store/cart'
import { DELIVERY_AREAS, DELIVERY_TIME_SLOTS, NIGERIAN_STATES } from '@/types/checkout'
...
export const CheckoutForm = ({ onOrderComplete }: CheckoutFormProps) => {
  ...
  const [availableTimeSlots, setAvailableTimeSlots] = useState(DELIVERY_TIME_SLOTS)
  ...
  const prepareOrderData = (formData: CheckoutFormData, paymentReference: string) => {
    ...
    return {
      items: filteredItems.map((item) => ({
        ...
        customCakeConfig: item.customizations
          ? {
              ...
              ...(item.customizations.message && { message: item.customizations.message }),
              ...(item.customizations.layers && {
                extraDetails: {
                  layers: item.customizations.layers,
                  ...(item.customizations.extras && { decorations: item.customizations.extras }),
                },
              }),
            }
          : undefined,
      ...
      pickup_instructions:
        formData.deliveryMethod === 'pickup' ? formData.deliveryInstructions : undefined,
  ...
  const handlePaymentSuccess = async (response: any, formData: CheckoutFormData) => {
    try {
      ...
      const response_order = await fetch(`${import.meta.env.VITE_API_URL}/v1/orders/guest`, {
        method: 'POST',
        headers: {
  ...
  const onSubmit = async (data: CheckoutFormData) => {
    try {
      ...
      const paymentConfig = paystackService.preparePaymentConfig(data, finalTotal, items)
  ...
  return (
    <form data-order onSubmit={handleSubmit(onSubmit)} className="space-y-8">
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm opacity-75">Delivered to your address</div>
                  </div>
                </div>
              </div>
            </label>
              ...
              >
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <div className="text-sm opacity-75">Pick up from our bakery</div>
                  </div>
                </div>
              </div>
            </label>
          ...
          <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div className="flex items-start">
              ...
              <div className="text-sm text-blue-800">
                <p className="font-medium">Delivery Information for {watchedState}</p>
                <p className="mt-1">
                  Delivery fee: {formatCurrency(deliveryFee)}
                  {deliveryFee === 0 &&
                    subtotal >=
                      (DELIVERY_AREAS[watchedState as keyof typeof DELIVERY_AREAS]
                        ?.freeDeliveryThreshold || 50000) && (
                      <span className="text-green-600 font-medium"> (Free delivery applied!)</span>
                    )}
                </p>
                {deliveryFee > 0 && (
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-start">
            ...
            />
            <span className="ml-3 text-sm text-gray-700">
              I would like to receive updates about new products and special offers via email
            </span>
          </label>
        </div>
      </div>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-3">
          ...
          <div className="border-t border-gray-200 pt-3 flex justify-between font-medium">
            ...
            <span className="text-gray-900 text-lg">{formatCurrency(finalTotal)}</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <span>
          {isProcessing ? 'Processing...' : `Pay ${formatCurrency(finalTotal)} with Paystack`}
        </span>
      </button>
